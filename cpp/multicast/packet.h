...
struct T_AMI_PKT_HDR
{
    uint32_t magic; // 'AVKS', 0x41564B53
    ...
    uint32_t crc32; // CRC32 of <AVKS-HDR + AVKS-PAYLOAD> (calc on crc32=0)
    uint32_t tick;  // monotonic tick
    uint32_t pl;    // packet length

    uint8_t payload[0];
};
#define AMI_PKT_HDR__SZ (sizeof(T_AMI_PKT_HDR))
...
struct FVMHeartbeat
{
    ...
    std::uint32_t fvm_status;
    std::uint32_t sensor_connection;
};
...
#define AMI_PKT_PLD_LEN (1408U)
#define AMI_UDS_PLD_LEN (AMI_PKT_PLD_LEN) // 16B * 88
#define AMI_UDP_PLD_LEN (AMI_PKT_PLD_LEN) // 16B * 88

#define AMI_PKT_LEN     (AMI_PKT_PLD_LEN + AMI_PKT_HDR__SZ)
#define AMI_UDS_PKT_LEN (AMI_UDS_PLD_LEN + AMI_PKT_HDR__SZ)
#define AMI_UDP_PKT_LEN (AMI_UDP_PLD_LEN + AMI_PKT_HDR__SZ)

/*-------------------------------------------------------------------------*/

#define AMI_PKT_MAGIC (0x41564B53U) // AVKS

#define AMI_PKT_TPN_MIN (1)
#define AMI_PKT_TPN_MAX (65535)

#define AMI_PKT_PSN_MIN (AMI_PKT_TPN_MIN - 1)
#define AMI_PKT_PSN_MAX (AMI_PKT_TPN_MAX - 1)

#define KB              ((uint32_t)1024U)
#define BLK_1K          ((uint32_t)(1 * KB))
#define BLK_ALIGN(a, b) ((uint32_t)((b) - ((a) % (b))))

#define AMI_PKT_BUF_LEN (AMI_PKT_LEN + BLK_ALIGN(AMI_PKT_LEN, BLK_1K))
...
inline std::uint32_t CalculateCrc32(const uint8_t *buf, std::size_t size, std::uint32_t crc32)
{
    crc32 = ~crc32;
    while (size--)
    {
